...
export default ({ slug_db }) => {
    ...
    return (
        <div className="bg-transparent md:translate-y-[-60px]">
            ...
            <div className="ml-[30px] flex gap-3 overflow-x-hidden scroll-auto py-5">
                {moviesList.map((item) => (
                    ...
                    >
                        <div
                            className="h-50 md:h-65 w-35 md:w-45 rounded-md hover:rounded-b-none bg-cover bg-center"
                            style={{
                                backgroundImage: `url(https://image.tmdb.org/t/p/original${item.poster_path})`,
                            }}
                        ></div>

                        <div className={`bg-[#222] p-2 rounded-b-md ${hoveredItemId === item.id ? 'opacity-100' : 'opacity-0'}`}>

            ...
            {item && (
                ...
                >
                    ...
                    <div className="min-h-screen flex items-center justify-center">
                        <div className="mx-auto flex flex-wrap max-w-full lg:max-w-7xl gap-3 justify-center">
                            {item.seasons && (
                            item.seasons.map((season) => (
                                <div key={season.id} className="text-center">
                                    <div 
                                        className="h-50 md:h-65 w-35 md:w-45 bg-cover bg-center rounded"
                                        style={{ backgroundImage: `url(https://image.tmdb.org/t/p/original/${season.poster_path})` }}>
                                    </div>
                                    <h1>{season.name}</h1>
                                    <p>{season.air_date?.substring(0, 4)}</p>
                                    <p>{season.vote_average.toFixed(1)} pontos</p>
